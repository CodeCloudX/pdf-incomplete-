...
def execute_tool(tool_id: str, file_paths: List[str], page_selections: Optional[Dict] = None, 
    ...
    try:
        ...
        if tool_id == "merge":
            ...
            # Merge always produces a single output PDF
            if isinstance(result, dict) and "output_files" in result:
                processed_files.extend([f["output_path"] if isinstance(f, dict) else f for f in result["output_files"]])
            ...
            if processed_files:
                return {
                    "status": "success",
                    "output_files": processed_files, # Should be a single PDF
                    "message": f"Successfully merged {len(file_paths)} files"
                }
            else:
                return {
                    "status": "error",
        ...
        # --- Unlock / Protect: single-file only ---
            ...
            if processed_files:
                return {
                    "status": "success",
                    "output_files": processed_files, # Should be a single PDF
                    "message": f"Successfully processed file with {tool_id}"
                }
            else:
                return {
                    "status": "error",
                    "output_files": [],
                    "message": f"{tool_id.capitalize()} operation failed"
                }
        ...
        # --- Split tool ---
            ...
            # For split, if there's only one output file (e.g., splitting a single page), don't zip it.
            # If there are multiple output files, or if the split option explicitly requested a zip, then zip.
            if all_split_files:
                split_option = tool_options.get('split_option', 'all') if tool_options else 'all'
                if len(all_split_files) > 1 or split_option == 'all': # 'all' implies zip for multiple pages
                    zip_file = create_zip_from_files(all_split_files, zip_prefix=f"split_{os.path.splitext(os.path.basename(file_paths[0]))[0]}")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                        "message": f"Split {len(file_paths)} files into {len(all_split_files)} parts"
                    }
                else: # Single output file, return as PDF
                    return {
                        "status": "success",
                        "output_files": all_split_files,
                        "message": f"Successfully split file"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- Rotate tool ---
            ...
            if all_rotated_files:
                # If multiple input files, or multiple output files from a single input (e.g., split and rotate), then zip.
                # Otherwise, return as individual PDF.
                if len(file_paths) > 1 or len(all_rotated_files) > 1:
                    zip_file = create_zip_from_files(all_rotated_files, zip_prefix=f"rotated_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_rotated_files,
                        "message": f"Successfully rotated file"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- Compress tool ---
            ...
            if all_compressed_files:
                if len(file_paths) > 1 or len(all_compressed_files) > 1:
                    zip_file = create_zip_from_files(all_compressed_files, zip_prefix=f"compressed_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_compressed_files,
                        "message": f"Successfully compressed file"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- PDF to Word tool ---
            ...
            if all_converted_files:
                if len(file_paths) > 1 or len(all_converted_files) > 1:
                    zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"pdf_to_word_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        ...
                        "message": f"Successfully converted PDF to Word"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- PDF to Excel tool ---
            ...
            if all_converted_files:
                if len(file_paths) > 1 or len(all_converted_files) > 1:
                    zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"pdf_to_excel_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_converted_files,
                        "message": f"Successfully converted PDF to Excel"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- PDF to PowerPoint tool ---
            ...
            if all_converted_files:
                if len(file_paths) > 1 or len(all_converted_files) > 1:
                    zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"pdf_to_ppt_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_converted_files,
                        "message": f"Successfully converted PDF to PowerPoint"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- PDF to JPG tool ---
            ...
            if all_converted_files:
                # PDF to JPG always produces multiple JPGs (one per page), so it should always be zipped.
                zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"pdf_to_jpg_files")
                return {
                    "status": "success",
                    "output_files": [zip_file],
        ...
        # --- PDF to Text tool ---
            ...
            if all_converted_files:
                if len(file_paths) > 1 or len(all_converted_files) > 1:
                    zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"pdf_to_text_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_converted_files,
                        "message": f"Successfully converted PDF to Text"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- OCR tool ---
            ...
            if all_converted_files:
                if len(file_paths) > 1 or len(all_converted_files) > 1:
                    zip_file = create_zip_from_files(all_converted_files, zip_prefix=f"ocr_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
                    ...
                    return {
                        "status": "success",
                        "output_files": all_converted_files,
                        "message": f"Successfully processed PDF with OCR"
                    }
            else:
                return {
                    "status": "error",
        ...
        # --- Default case for other tools (e.g., single PDF input, single PDF output) ---
        else:
            ...
            for file_path in file_paths:
                ...
                try:
                    ...
                    # For tools like compress, rotate, etc., the tool_func itself should handle pages
                    res = tool_func(file_path, pages=pages, **tool_options) # Pass all tool_options
            ...
            if processed_files:
                # If multiple input files, or multiple output files from a single input, then zip.
                # Otherwise, return as individual PDF.
                if len(file_paths) > 1 or len(processed_files) > 1:
                    zip_file = create_zip_from_files(processed_files, zip_prefix=f"{tool_id}_files")
                    return {
                        "status": "success",
                        "output_files": [zip_file],
